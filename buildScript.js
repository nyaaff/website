...
function buildDir(dir) {
    ...
    filesNDirs.forEach(fileOrDir => {
        if (fileOrDir.isDirectory()) {
            ...
            return buildDir(path.join(dir, fileOrDir.name));
        }
        if (fileOrDir.name.match(/\.ejs$/)) {
            const from = path.join(dir, fileOrDir.name);
            const to = path.join(path.join(dir, fileOrDir.name.replace(/\.ejs$/, '.html')).replace(srcPath, buildPath))
            const html = ejs.render(fs.readFileSync(from).toString(), data[fileOrDir.name], {views: [srcPath]});
            fs.mkdirSync(to.replace(path.basename(to), ''), {recursive: true});
